...
def _corr_convolve_fast(x, y, mode='same', method='auto'):
    ...
    M_fft = np.clip(nextpow2(M), 2, 2048)
...
def fill_at(array, point, other, mode='replace', allow_exceed=True):
    ...
    if mode == 'replace':
        ...
        array[index] = np.max([array[index], other[index_other]], axis=0)
    else:
...
def assert_equal(first, second, rtol=1e-5, atol=1e-8):
    ...
    if first.shape != second.shape:
        raise Exception("Shape differ: %s != %s" % (first.shape, second.shape))
    try:
...
def uarray(array, std_array):
    return np.array([ufloat(n, s) for n, s in zip(array, std_array)])


def uarray_n(array, dtype=np.float64):
    if isinstance(array[0], UFloat):
        return np.array([k.n for k in array], dtype=dtype)
    return array
